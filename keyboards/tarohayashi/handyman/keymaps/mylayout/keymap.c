...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [CLIPSTUDIO] = LAYOUT(
        ...
        LSG(KC_I), G(KC_U), G(KC_T),  G(KC_D), G(KC_C),
        KC_X, G(KC_0), G(KC_Z), KC_9, G(KC_V),
        KC_O, KC_S, KC_Y, KC_I, LAG(KC_0),
        KC_H, LT(LIGHT, KC_U), LALT_T(KC_E), GUI_T(KC_B), LSFT_T(KC_P),
        // LEVER
        XXXXXXX, KC_T, KC_G, KC_J, XXXXXXX,
        // ENCODER
        LGUI(KC_KP_PLUS), LGUI(KC_KP_MINUS),   KC_LBRC,KC_RBRC,  KC_MINS, KC_QUOT
  ),
  [PHOTOSHOP] = LAYOUT(
        G(KC_S), G(KC_C), G(KC_V), G(KC_X), KC_BSPC,
